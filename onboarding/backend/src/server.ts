...
io.on('connection', (socket) => {
    ...
    socket.on(
        'task-update',
        async (data: { userId: string; taskId: string; status: string }) => {
            try {
                // Update task in database
                const updatedTask = await prisma.task.update({
                    where: { id: data.taskId },
                    data: { status: data.status as any }, // Cast to enum type
                });
                ...
                const newBadges = await gamificationService.checkBadgeUnlocks(
                    data.userId
                );
    ...
    socket.on(
        'progress-update',
        async (data: { userId: string; step: number; percentage: number }) => {
            try {
                ...
                logger.info(
                    `Progress updated for user ${data.userId}: step ${data.step}, ${data.percentage}%`
                );
            } catch (error) {
    ...
    socket.on(
        ...
        async (data: { userId: string; message: string }) => {
            try {
                ...
                io.to(`user-${data.userId}`).emit('ai-error', {
